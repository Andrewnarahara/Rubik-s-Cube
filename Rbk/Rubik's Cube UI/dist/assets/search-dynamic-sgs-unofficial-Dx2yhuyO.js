import{p as H,b as N,j as y}from"./index-BJK48WJQ.js";import{p as E}from"./parseSGS-BMJCnQw7.js";let J=null;async function pB(){return J??(J=K())}async function K(){return E(await H.fto.kpuzzle(),`SubgroupSizes 24 12 11 12 12 11 10 9 11 8 10 9 7 8 10 10 6 9 8 5 7 6 5 4 4 8 7 3 6 6 360
...
`)}function h(R){if(R<2)return R;let B=1;for(let L=2;L<=R;L++)B*=L;return B}function m(R,B){if(B<0||B>R)return 0;if(B===0||B===R)return 1;let L=1;for(let F=0;F<B;F++)L=L*(R-F)/(F+1)|0;return L}function P(R){R=R.slice();let B=R.length,L=h(B-1),F=0;for(;B>1;){B--;let U=R[0];F+=U*L;for(let l=0;l<B;l++){let g=R[l+1];R[l]=g-(g>U)}L/=B}return F}function Q(R,B){let L=[],F=h(B-1);for(let U=0;U<B;U++)L[U]=R/F|0,R%=F,F/=B-1-U;for(let U=B-2;U>=0;U--)for(let l=U+1;l<B;l++)L[l]+=+(L[l]>=L[U]);return L}function b(R){let B=R.length,L=0;for(let F=0;F<B-1;F++)for(let U=F;U<B;U++)R[F]>R[U]&&(L^=1);return L}function V(R,B){let L=[],F=h(B-1)/2,U=0;for(let l=0;l<B-1;l++)L[l]=R/F|0,R%=F,F/=B-1-l;L[B-1]=0;for(let l=B-2;l>=0;l--)for(let g=l+1;g<B;g++)L[g]>=L[l]?L[g]++:U^=1;return U===1&&([L[B-2],L[B-1]]=[L[B-1],L[B-2]]),L}function x(R){return P(R)>>1}function T(R){let B=R.length,L=0;for(let g=0;g<B;g++)L+=+(R[g]===1);let F=B-L;if(F===0||L===0||B===1)return 0;let U=m(B-1,L),l=0;for(let g=0;F>0&&L>0&&B>1;g++)B--,R[g]===0?U=U*--F/B:(l+=U,U=U*L--/B);return l}function W(R,B,L){let F=L-B,U=m(L-1,B),l=[],g=L-1;for(let A=0;A<g;A++)L--,R<U?(l.push(0),U=U*--F/L):(l.push(1),R-=U,U=U*B--/L);return l.push(B),l}function p(R,B){let L=[];for(let F=0;F<B.length;F++)L[F]=R[B[F]];return L}function f(R,B){let L=[];for(let F=0;F<B;F++)L[F]=F;for(let F=0;F<R.length;F++)L[R[F]]=R[(F+1)%R.length];return L}function a(R,B){let L=Array(B).fill(0);for(let F in R)L[F]=R[F];return L}function X(R,B){let L=Array(8).fill(0);for(let F=0;F<8;F++)L[F]=(R[1][F]+B[1][F])%3;return[p(R[0],B[0]),L]}let Y=[f([0,1,4],12),a({0:2},8)],Z=[f([1,2,5],12),a({1:2},8)],$=[f([2,3,6],12),a({2:2},8)],BB=[f([3,0,7],12),a({3:2},8)],LB=[f([9,8,4],12),a({4:2},8)],RB=[f([10,9,5],12),a({5:2},8)],FB=[f([11,10,6],12),a({6:2},8)],lB=[f([8,11,7],12),a({7:2},8)];Q(0,12),Array(8).fill(0);let s=[Y,Z,$,BB,LB,RB,FB,lB],UB=["UL","U","UR","F","L","B","R","D"],_=[0,1,0,1,1,0,1,0];function DB(R){let B=["0","","'"];return R.map(([F,U])=>UB[F]+B[U]).join(" ")}function gB(){let R=V(Math.floor(y(h(12))/2),12),B=Array(8);for(let L=0;L<8;L++)B[L]=y(3);return[R,B]}function AB(){return eB(gB())}function eB(R){let B=tB(R),L=[j(),S()],F=[nB(),oB()],U=[z(),iB()],l=[fB(),rB()],g=uB(B,L,F),A,D=new Set,r=+new Date;for(let o=0;o<25;o++){let{value:t,done:n}=g.next(),i=R;for(let[u,I]of t)for(let w=0;w<I;w++)i=X(i,s[u]);if(D.has(i.toString()))continue;D.add(i.toString());let O=x(i[0].slice(0,8)),v=0;for(let u=0;u<4;u++)v+=i[1][u]*3**u;let q=[O,v],G=A?A.length-t.length-1:999999,c=aB(q,U,l,G);if(c!==void 0&&((A===void 0||A.length>t.length+c.length)&&(A=t.concat(c)),new Date-r>300))break}return A}function tB(R){let B=0;for(let l=0;l<4;l++)B+=R[0].indexOf(l+8)*12**l;let L=0;for(let l=0;l<4;l++)L+=R[1][l+4]*3**l;let F=R[0].map(l=>Math.max(-1,l-8)),U=T(F.map(l=>+(l>=0)))*2+b(F.filter(l=>l>=0));return[B,L+81*U]}let e={};function M(){if(e.phase1cm)return e.phase1cm;let R=[];for(let B=0;B<81;B++){R[B]=Array(8);let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B][0]=R[B][1]=R[B][2]=R[B][3]=B;for(let F=0;F<4;F++)L[F]=(L[F]+2)%3,R[B][4+F]=L[0]+L[1]*3+L[2]*9+L[3]*27,L[F]=(L[F]+1)%3}return e.phase1cm=R}function rB(){if(e.phase1cp)return e.phase1cp;let R=Array(81);for(let B=0;B<81;B++){let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B]=(L[0]!==0)+(L[1]!==0)+(L[2]!==0)+(L[3]!==0)}return e.phase1cp=R}function j(){if(e.phase1em)return e.phase1em;let R=[];for(let L=0;L<12;L++){R[L]=[];for(let F=0;F<8;F++)R[L][F]=s[F][0].indexOf(L)}let B=Array(12**4);for(let L=0;L<12**4;L++){B[L]=Array(8);for(let F=0;F<8;F++){let U=0;for(let l=0;l<4;l++)U+=R[Math.floor(L/12**l)%12][F]*12**l;B[L][F]=U}}return e.phase1em=B}function nB(){return e.phase1ep?e.phase1ep:e.phase1ep=d(j(),[8+12*(9+12*(10+12*11))])}function S(){if(e.phase1sm)return e.phase1sm;const R=m(12,4);let B=[];for(let U=0;U<R;U++){B[U]=[];let l=W(U,4,12),g=[];for(let A=0,D=0;A<12;A++)l[A]===0?g[A]=-1:g[A]=D++;for(let A=0;A<8;A++){let D=p(g,s[A][0]),r=p(l,s[A][0]),o=b(D.filter(t=>t>=0));B[U][A]=T(r)*2+o}}let L=M(),F=[];for(let U=0;U<R;U++)for(let l=0;l<81;l++){let g=F[l+81*(2*U)]=[],A=F[l+81*(2*U+1)]=[];for(let D=0;D<8;D++)g[D]=L[l][D]+81*B[U][D],A[D]=L[l][D]+81*(B[U][D]^1)}return e.phase1sm=F}function oB(){return e.phase1sp?e.phase1sp:e.phase1sp=d(S(),[0])}function iB(){if(e.phase2cm)return e.phase2cm;let R=M(),B=Array(81);for(let L=0;L<81;L++)B[L]=R[L].slice(4,8);return e.phase2cm=B}function z(){if(e.phase2em)return e.phase2em;const R=8,B=h(R)/2;let L=Array(B),F=[0,1,2,3,4,5,6,7];for(let U=0;U<B;U++){L[U]=Array(4);for(let g=0;g<4;g++){let A=p(F,s[g][0].slice(0,8));L[U][g]=x(A)}if(U===B-1)break;let l=0;do for(let g=R-2;g>=0;g--){if(F[g]>F[g+1])continue;let A=g+1;for(let D=A;D<R;D++)F[D]>F[g]&&(A=D);[F[g],F[A]]=[F[A],F[g]],l^=1;for(let D=0;g+1+D<R-1-D;D++,l^=1)[F[g+1+D],F[R-1-D]]=[F[R-1-D],F[g+1+D]];break}while(l!==0)}return e.phase2em=L}function fB(){return e.phase2ep?e.phase2ep:e.phase2ep=d(z(),[0])}function d(R,B){let L=R.length,F=R[0].length,U=Array(L).fill(-1),l=B.slice(),g=[],A=0;for(;l.length>0;){g.length=0;for(let D of l)if(U[D]===-1){U[D]=A;for(let r=0;r<F;r++){let o=R[D][r];for(;o!==D;)g.push(o),o=R[o][r]}}[l,g]=[g,l],A+=1}return U}function aB(R,B,L,F){F=F||999999;let U=R.length,l=0;for(let g=0;g<U;g++)l=Math.max(l,L[g][R[g]]);for(;l<=F;){let g=C(R,B,L,l,-1);if(g!==void 0)return g;l++}}function C(R,B,L,F,U){let l=R.length,g=B[0][0].length,A=0;for(let D=0;D<l;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0)return[];if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===U||D<U&&_[D]===_[U])continue;let r=R.slice();for(let t=0;t<l;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=C(r,B,L,F-1,D);if(t!==void 0)return[[D,o]].concat(t);for(let n=0;n<l;n++)r[n]=B[n][r[n]][D];o++}}}}function*uB(R,B,L){let F=R.length,U=0;for(let l=0;l<F;l++)U=Math.max(U,L[l][R[l]]);for(;;)yield*k(R,B,L,U,-1),U++}function*k(R,B,L,F,U){let l=R.length,g=B[0][0].length,A=0;for(let D=0;D<l;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0){yield[];return}if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===U||D<U&&_[D]===_[U])continue;let r=R.slice();for(let t=0;t<l;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=k(r,B,L,F-1,D);for(;;){let{value:n,done:i}=t.next();if(i)break;yield[[D,o]].concat(n)}for(let n=0;n<l;n++)r[n]=B[n][r[n]][D];o++}}}}async function _B(){return new N(DB(AB()))}export{_B as getRandomRediCubeScramble,pB as sgsDataFTO};
